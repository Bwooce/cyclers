"""CCR4BP whisker/manifold-direction extraction tests for Jupiter-Io-Ganymede (#696).

Applies `#691`'s segment-anchored discrete-QR/CLV extraction
(``search.ccr4bp_whisker``, reused UNMODIFIED) to the Io-Ganymede 4:1 torus
(see ``test_ccr4bp_torus_io_ganymede.py`` for why the tractable base orbit is
spacecraft:Io = 4:1, interior, not the 1:4 exterior orbit that collides with
Ganymede's own orbit). Mirrors `#691`'s own go/no-go discipline: run the
perturbation-robustness diagnostic on this NEW system before trusting the
cheap segmented-CLV direction for the downstream manifold-globalization +
heteroclinic search (`#694`'s modules, run in the discovery-attempt script).

Live-observed headline result (`#696`): this system's perturbation-robustness
swings are SMALLER than JEG's own (worst one-shot ~1.3 deg here vs JEG's ~3.4
deg; worst segmented ~0.03 deg here vs JEG's ~0.008-0.1 deg, same order) --
plausible given `mu_gan` is essentially identical to JEG's own, but this base
orbit (interior, near-circular, comfortably clear of Ganymede) is less
eccentric and less strongly perturbed than JEG's Europa 3:4 orbit. Both
methods are trustworthy here; the expensive full Kumar bundle-solve whisker
...
def test_amplification_diagnostics_physical_torus(
    ...
    Live-observed: |lam_u| ~15.08-15.09 across phases (much narrower spread
    than JEG's 9-1700 range -- this near-circular interior orbit's stability
    structure barely varies with phase), separation ratio ~15."""
    checked = 0
...
def test_perturbation_robustness_physical_torus(
    ...
    Live-observed worst-case swings (5 seeds, 3 phases x 2 branches): one-shot
    up to ~1.28 deg; segmented up to ~0.027 deg -- BOTH methods trustworthy,
    same conclusion as JEG (`#691`)."""
