"""CCR4BP whisker/manifold-direction extraction tests for Jupiter-Europa-Callisto (`#703`).

Applies `#691`'s segment-anchored discrete-QR/CLV extraction
(``search.ccr4bp_whisker``, reused UNMODIFIED) to the Europa-Callisto 4:1
torus (see ``test_ccr4bp_torus_europa_callisto.py`` for why the tractable
base orbit is spacecraft:Europa = 4:1, interior). Mirrors `#691`'s own
go/no-go discipline: run the perturbation-robustness diagnostic on this NEW
system before trusting the cheap segmented-CLV direction for the downstream
manifold-globalization + heteroclinic search (`#694`'s modules, run in the
discovery-attempt driver script).

Live-observed headline result: this torus's one-period unstable eigenvalue is
MUCH larger than every other system this project has built --
``|lam_u| ~ 174.83-174.83`` across the 12 checked phases (dominant real
hyperbolic pair, separation ratio == ``|lam_u|`` since no other eigenvalue
approaches its magnitude at any checked phase) -- an order of magnitude
stronger than JEG's own ~6-13 range or Umbriel-Titania's own ~6-13.5 range,
consistent with the interior 4:1 orbit's tighter, faster dynamics near
Jupiter. The perturbation-robustness diagnostic is CLEANLY trustworthy here
despite the much stronger amplification: worst one-shot swing ~1.28 deg,
worst segmented swing ~0.0255 deg (both far below the 10-degree/0.1-degree
go/no-go bars this project uses, and the segmented method is, as expected,
tighter than the one-shot method by roughly an order of magnitude on the
unstable branch). The expensive full Kumar bundle-solve whisker construction
is NOT indicated.
"""
...
def _resonant_symmetric_orbit(
    ...
    """Identical test-only scaffolding to #690/#691/#694/#695/#696/#701's own test modules."""
...
def test_amplification_diagnostics_physical_torus(
    ...
    Live-observed: |lam_u| ~174.83-174.83 across phases -- much stronger
    amplification than every other CCR4BP system this project has built,
    consistent with the interior 4:1 orbit's tighter, faster dynamics near
    Jupiter -- separation ratio equal to |lam_u| (no competing eigenvalue at
    any checked phase)."""
    checked = 0
    for theta1 in np.linspace(0.0, 2 * np.pi, 4, endpoint=False):
        for theta2 in np.linspace(0.0, 2 * np.pi, 3, endpoint=False):
            ...
            assert 150.0 < abs(diag.lam_u) < 200.0, (theta1, theta2, diag.lam_u)
            assert diag.lam_s is not None
            assert diag.lam_u * diag.lam_s == pytest.approx(1.0, abs=5e-3)
            assert diag.separation_ratio > 100.0, (theta1, theta2, diag.separation_ratio)
            checked += 1
    assert checked == 12
...
def test_perturbation_robustness_physical_torus(
    ...
    Checks three representative phases, mirroring #691's/#695's/#696's/
    #701's own discipline exactly. Live-observed worst-case swings (5 seeds,
    3 phases x 2 branches): one-shot ~1.28 deg, segmented ~0.0255 deg -- BOTH
    methods trustworthy, despite this system's much stronger one-period
    amplification than any prior system built."""
