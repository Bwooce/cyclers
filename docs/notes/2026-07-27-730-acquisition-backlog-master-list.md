...
|---|---|---|---|---|---|
| 1 | Blazevski, D. & Ocampo, C., "Periodic Orbits in the Concentric Circular Restricted Four-Body Problem and Their Invariant Manifolds," *Physica D* 241(13):1158–1167 (2012) | **DOI `10.1016/j.physd.2012.03.008`** (confirmed) | **HIGH (recurring)** | `#710`, `#722` §7a (Kumar's own Eq. 2-3 cite it), `#727` (HIGH), `#728`-oberon-survey ("corroborated by 3 digests"), `#728`-kumar-2026-fast-multishooting (ref 17) — **5 independent flags** | THE original CCR4BP model-definition paper. This project's own `core/ccr4bp.py` and the new `core/crnbp.py` both trace their equations of motion through the Kumar-lineage papers back to this one paper, which the project has never itself held — a genuine grounding-chain gap for the single most load-bearing model this project's Uranian/Jovian CCR4BP discovery arc (`#689`-`#708`, `#714`-`#728`) depends on. |
...
| 4 | Baresi, N., Olikara, Z.P. & Scheeres, D.J., "Fully Numerical Methods for Continuing Families of Quasi-Periodic Invariant Tori in Astrodynamics," *J. Astronaut. Sci.* 65, 157–182 (2018) | **DOI `10.1007/s40295-017-0124-6`** (confirmed) | HIGH | `#722` (HIGH) | The specific continuation-method paper (phase-condition/parametrizing-equation machinery) both Baresi/Owen/Scheeres TCP papers (already in corpus) cite for their own torus-continuation algorithm. |
| 5 | Negri, R.B. & Prado, A.F.B.A., "Generalizing the Bicircular Restricted Four-Body Problem," *JGCD* 43(6):1173–1179 (2020) | **DOI `10.2514/1.G004848`** (confirmed) | HIGH | Negri-Prado-2022-digest | The N=4 special case this project's already-acquired Negri & Prado 2022 CRNBP paper explicitly generalizes; corrects Huang 1960's BCR4BP indirect term — the exact term `core/bcr4bp.py`'s own indirect-term derivation should be checked against. |
