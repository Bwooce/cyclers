...
- **Does the project's DOP853 integrator hold corrector tolerance at the bifurcation?** Near a saddle-center, the monodromy has multipliers near (+1, +1, +1, +1, …) — clustered eigenvalues are ill-conditioned. The existing `monodromy` runs at rtol=atol=1e-12 (the project's standard). **Unknown** whether the eigenvalue separation will survive at the cluster point. Mitigation: cross-check the multiplier via the trace/determinant invariants (the characteristic polynomial coefficients should match a known structure for a Hamiltonian monodromy). Add this as a check in P1.3. **Update 2026-06-17 (#372 P372.3, see `data/372_stm_mode_crosscheck.jsonl`):** Pellegrini & Russell 2016 (JGCD, DOI 10.2514/1.G001920, eq. 17 p. 3) quantifies this as the "δt/∂X contamination" — under variable-step DOP853 the step size is IC-dependent and the variational equations miss a term. Cross-checked at the (3,2) post-saddle-center anchor (C=3.14180): |λ_max| disagrees by 7.2e-4 (0.07%, well under the 1% acceptance), so the Phase 1 |λ_max|=2.5e5 detection is NOT compromised. **However**, the variable-step path *does* smear the trivial-pair from (+1, +1) to (1.0035, 0.9965) — 0.35% — whereas the new fixed-path mode (`stm_mode='fixed_path'` in `cr3bp.propagate` + `bifurcation_detector.monodromy`, commit `d901c90`) keeps the pair coincident at +1 to 7 digits. For k=1 saddle-center detection via secondary-pair distance to +1, **`detect_saddle_center_bracket` should be re-run with `stm_mode='fixed_path'` in Phase 2** to drop the trivial-pair noise floor below the secondary signal. lambda_max-based detection (k≥2) is unaffected at this tolerance.
