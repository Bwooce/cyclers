...
- `load_m6b_entries() -> list[dict]` — read `data/seed_cyclers.yaml`, filter to `model_assumption == "circular-coplanar"` AND `trajectory_regime == "ballistic"` AND `primary in (None, "Sun")` AND the entry passed V1 in the M6a regression record. Returns the raw dict per entry (caller turns it into a `Cycler` via `construct_real_ephemeris_cycler`).
