...
## Headline counts (as of 2026-06-01, post task #66)

| Class                       | Count | Meaning                                                              |
| --------------------------- | ----: | ------------------------------------------------------------------- |
| **migrated**                |     3 | has `trajectory.segments` (s1l1 exemplar + Aldrin out/in)           |
| **legacy** (`legs[]` only)  |   203 | still on the flat top-level `legs[]`                                |
...
| **topology-gap** |   202 | every present leg has `tof_days`, but `len(legs) < n_encounters` from `sequence_canonical`          | needs s1l1-style `data_gaps[]` markers, NOT a naive copy |
...
lossless Aldrin migrations below, **zero** legacy entries are "full." All 202
...
Both were verified **signature-preserving**: `signature_hash` for each is
bitwise-identical before/after, and the aggregate sha1 over all 202 entry
signatures is unchanged (`25d8a711…`). The loader's `_segments_as_legs` reads
the new `trajectory.segments` and produces the identical canonical signature it
previously derived from `legs[]`.

## Ranked worklist — remaining legacy entries (202 topology-gap + 1 sparse)
...
1. **`mcconaghy-2006-em-k2`** — 2 legs both with `tof_days`, named "Notable
   Two-Synodic" cycler, sibling of the s1l1 exemplar. Best next candidate:
   `E-E-M-M` topology means the intermediate Earth loop is still a gap, so it
   lands as a 2-segment-attested + `data_gaps[]` migration (the exemplar shape),
   not "full." Migrate when the AIAA 2002-4420 / JSR 2006 leg decomposition is
   in hand.
2. **`russell-ch4-*` family (14 entries)** — each has a symmetric 2-leg
   (`t_out` / `t_in`) record from Russell 2004 Tables 4.x, but `E-E-M-M`
   topology + explicitly per-leg (a,e) that are `null` (the notes state each
   generic-return arc has its own `(a,e)`). Migrate as 2 attested segments +
   `data_gaps[]` for the intermediate loop and the per-segment elements.
3. **`mcconaghy-2005-em-case1`, `mcconaghy-2005-em-u0l1`** — single E→M leg with
   orbit-level `(a,e)` present; `E-E-M-M` topology. Return + loop are gaps.
4. **`russell-ocampo-*` family (~185 entries, the bulk)** — single E→M transit
   ...
   markers. Defer until the multi-rev Lambert solver (task #54) can *derive* the
   missing loop arcs, at which point they become `kind: "derive"` gaps that get
   filled programmatically rather than from literature.
5. **`aldrin-4-3-2-establishment`** (sparse; `analytic-ephemeris`) — 1 leg with
   no `tof_days`. Source-gated on Rogers 2012 Tables 3/4 extraction (also blocks
   `v_infinity_leveraging_dv_kms`). Lowest priority.
...
legacy row. Today that is 203 entries (down from 205).
