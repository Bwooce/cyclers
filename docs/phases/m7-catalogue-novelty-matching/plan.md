...
No edits to M0–M6a source bodies. The catalogue YAML at `data/catalogue.yaml` is **read-only from M7 source code** under CI; the on-disk writeback path exists and is exercised by integration tests writing to `tmp_path`, never to the canonical file.
...
- `serialise_entry_yaml(entry) -> str` — produce the YAML representation of a `CatalogueEntry` for writing back to `data/catalogue.yaml`. Round-trip stable: `serialise_entry_yaml(load_catalog().by_id["s1l1-2syn-em-cpom"])` == the on-disk YAML block, modulo comment formatting.
...
| Site / `cyclers.space` integration | **Live (deployed)** per `docs/overview.md` §4 | The public site reads `data/catalogue.yaml` directly. M7's writeback path produces YAML the site already understands; no M7-side site code is needed. |
...
| `load_catalog(path) -> Catalog` | Load + index `data/catalogue.yaml` | `data/catalog.py` (NEW) |
...
- [ ] `## Hand-off to M8` section appended to `phases/m7-catalogue-novelty-matching/todo.md` covering:
  ...
  - Confirmation that `data/catalogue.yaml` was NOT modified by any M7 test (the CI gate writes only to `tmp_path`).
