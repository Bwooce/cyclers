...
### (c) Per-leg DSM (reuse close_epoch_locked's built-in DSM execution)
`close_epoch_locked` ALREADY executes a prescribed per-leg DSM via `trajectory_spec.dsm_specs` (`DSMSpec(leg_index, fraction_along_leg, delta_v_kms)`): arc1 propagates the ballistic-Lambert departure to `fraction_along_leg * tof`, applies the prescribed heliocentric Δv vector, arc2 forward-propagates to the arrival epoch; the resulting V∞-mismatch feeds the closure residual and the prescribed |Δv| is the DSM cost (`per_dsm_delta_v_kms`). So the engine maps each leg's decision sub-vector `(eta_i, dvx_i, dvy_i, dvz_i)` directly to a `DSMSpec` and reuses `close_epoch_locked` as the **entire** evaluation oracle — no separate transcription. `eta_i→0` / `|dv_i|→0` ⇒ ballistic leg (no DSMSpec emitted for that leg). The DSM Δv is thus a first-class decision variable the optimiser trades against closure/continuity. (Note: this is `close_epoch_locked`'s forward-propagate DSM executor, NOT `evaluate_dsm_leg`'s Lambert-targeting transcription — the latter stays the standalone #307 Task-2 tool, unused here.)
