...
- **`known-class-member` (the third status, added 2026-06-25, #444):** a fourth `our_status` value between `known-reproduction` and `candidate-novel`, for a **computed member of a published CLASS that is not a literal copy of any specific published orbit.** Use it when: (a) no `first_published` can be set (no paper tabulates *this* orbit — e.g. wrong mass ratio, no matching IC), yet (b) the orbit's *type* is published — cited via `corroborating_sources`, not `first_published`. It is **NOT novelty-claimable** (the class is known, so it can never become `candidate-novel`/`verified-novel`) and **NOT** a `known-reproduction` (no specific published orbit to reproduce). Canonical case: the C21 stable 3D Earth-Moon cycler — a computed member of the Antoniadou & Libert (2019) published spatial-resonant *class* (µ=0.001 planetary; no Earth-Moon IC), the 3D out-of-plane extension of the catalogued planar `ross-rt-em-cycler-21-2025`. Set `our_status: known-class-member` explicitly on the row (it is not inferable from `first_published`, which is absent); the `corroborating_sources` carry the class citation. Surfaced by the §[fast spatial-novelty pre-filter](../src/cyclerfinder/genome/spatial_novelty_prefilter.py): out-of-plane extensions of known planar families are class-published by the vertical-bifurcation mechanism and must be routed here, not to the discovery gauntlet.
