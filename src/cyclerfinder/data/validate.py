...
_LEVEL_EVIDENCE: dict[tuple[str, str], str] = {
    ...
    ("europa-3-4-crnbp-torus-jupiter-2026", "V1"): (
        "spec §14 V1-EQUIVALENT (task #738, closing the #735 Sec 8 gap left open at "
        "writeback #736): the literal V1 gate (lamberthub izzo+gooding leg agreement + "
        "Kepler forward re-propagation) has no analogue for a bare CRNBP quasi-periodic "
        "torus (no Lambert legs, no manifold connection either -- unlike the Umbriel-"
        "Titania row, there is no connection here at all, just the torus's own short-time "
        "flow-consistency claim). The accepted same-CLASS substitute, mirroring the "
        "Umbriel-Titania row's own precedent exactly: search.crnbp_torus_ghost_guard."
        "radau_ghost_guard cross-checks variational_crnbp_torus._independent_closure's own "
        "DOP853 short-time flow-consistency check under an INDEPENDENT integrator (Radau), "
        "at the IDENTICAL (theta1, theta2) sample anchors (same RNG seed 0xC0FFEE, same "
        "dt_frac/n_samples) -- agreeing to integrator_delta_max_km=7.112659620276295e-08 km "
        "(data/found/738_crnbp_torus_radau_ghost_guard/result.json:check."
        "integrator_delta_max_km), seven orders of magnitude below the Umbriel-Titania row's "
        "own 1.0 km gate. CONSISTENCY gate (same inputs -- the same torus-point states and "
        "the FULL coupling-term-included crnbp_eom equations of motion -- different "
        "algorithm; only the integrator differs), per this module's own #197 gate-"
        "classification convention (validate.py module docstring) -- the SAME class as the "
        "literal V1 gate's own lamberthub cross-check and as the Umbriel-Titania row's own "
        "V1 entry, not an upgrade to an independence gate. shared with primary path: same "
        "torus-point departure states and crnbp_eom equations of motion; only the "
        "integrator differs. The #726/#729 real-SPICE (jup365.bsp) model-shadowing "
        "evidence (independently re-derived inputs, not just a different algorithm) is "
        "the genuinely independence-flavored companion evidence, reported in "
        "crnbp_provenance.real_ephemeris_evidence but not itself gating this V1 claim. "
        "#702's anchoring lesson is adapted, not copied: this object "
        "class has no eigenvector/manifold-lobe sign ambiguity (evaluate_torus_state is a "
        "deterministic, single-valued Fourier-series evaluation), so the analogous hazard "
        "guarded against is a MISMATCHED random sample set, not a sign flip -- "
        "radau_ghost_guard reuses _independent_closure's own sampling code path exactly and "
        "self-checks (reproduces_reported_closure) that its DOP853 side reproduces the "
        "corrector's own stored closure_residual before trusting any Radau delta (see the "
        "module's own docstring). NOT V2 (no multi-lap periodic structure exists for a bare "
        "torus). NOT V3 (no budget-bounded correction burn has been run at any of the #729 "
        "real-ephemeris near-miss epochs -- there is no connection to correct in the first "
        "place). tests/search/test_crnbp_torus_ghost_guard.py (ghost-guard machinery + the "
        "physical-torus positive control) + tests/search/test_variational_crnbp_torus.py "
    ...
    # stronger x0/period/Jacobi/stability self-consistency check passes) + #801 (2026-08-09,
    # completing the writeback for 1-2e/7-3a, originally excluded by a k_par-vs-k_perp selection
    # bug in the stability-index reproduction check -- see earth_moon_resonant_families.py's
    # StabilityIndex docstring). All 9 raw-admitted rows are now written back: 6 resonant_po (no
    # lunar encounter, Casoliva's own periselene r_pM outside the lunar SOI/Hill radius -- see
    # each row's own orbit_class comment) + 3 cycler (genuine lunar encounter, r_pM well inside
    # SOI/Hill radius).
    ...
    ("casoliva-1-2e-em-resonant-po-2010", "V1"): (
        "spec §14 V1 (#801, reusing #780's own gate module "
        "earth_moon_resonant_families.table3_gate_report/recover_table3_row, fixed this task's "
        "own k_par-vs-k_perp selection bug): "
        "same-model planar CR3BP full-state Newton reproduction of Casoliva 2010 (JGCD 33(5), "
        'Table 3, p.1630) row "1-2e" -- x0 reproduces to 3.00e-09 relative, period to 4.06e-08 '
        "relative, Jacobi constant to 6.41e-09 relative, AND Casoliva's own printed Eq. 6-8 "
        "full-period stability index k reproduces to 3.27e-08 relative (her own STABLE verdict, "
        "|k|<2, is reproduced -- this row's own printed k is a k_perp match, not the naive "
        "max(|k_par|,|k_perp|) block, see StabilityIndex's own docstring) -- AND an independent "
        "Radau integrator cross-check preserves the "
        "Jacobi constant to dJ=3.64e-14 over one period (radau_ok=True). "
        "closure_residual=8.49e-12. Independent DERIVE re-verification: direct dense propagation "
        "(scipy solve_ivp DOP853, rtol=atol=1e-12) "
        "of the corrected orbit confirms Casoliva's own printed periselene/perigee columns "
        "(r_pM/r_pE) to <1e-3 relative, the evidence this row's own orbit_class determination "
        "rests on. NOT V2 (no multi-lap bounded-drift run has been performed this task). "
        "docs/notes/2026-08-07-780-earth-moon-casoliva-families-gate.md + "
    ...
    ("casoliva-7-3a-em-cycler-2010", "V1"): (
        ...
        "own k_par-vs-k_perp selection bug): "
        "same-model planar CR3BP full-state Newton reproduction of Casoliva 2010 (JGCD 33(5), "
        'Table 3, p.1630) row "7-3a" -- x0 reproduces to 1.04e-06 relative, period to 1.41e-07 '
        "relative, Jacobi constant to 2.85e-06 relative, AND Casoliva's own printed Eq. 6-8 "
        "full-period stability index k reproduces to 3.78e-04 relative (her own STABLE verdict, "
        "|k|<2, is reproduced -- this row's own printed k is a k_perp match, not the naive "
        "max(|k_par|,|k_perp|) block, see StabilityIndex's own docstring) -- AND an independent "
        "Radau integrator cross-check preserves the "
        "Jacobi constant to dJ=1.90e-12 over one period (radau_ok=True). "
        "closure_residual=1.17e-09. Independent DERIVE re-verification: direct dense propagation "
        "(scipy solve_ivp DOP853, rtol=atol=1e-12) "
        "of the corrected orbit confirms Casoliva's own printed periselene/perigee columns "
        "(r_pM/r_pE) to <1e-3 relative, the evidence this row's own orbit_class determination "
        "rests on (WELL INSIDE the lunar SOI, unlike this task's own dispatch-note prose which "
        "wrongly estimated this row as outside it -- a transcription error in that prose, "
        "corrected against the actual vendored table data). NOT V2 (no multi-lap bounded-drift "
        "run has been performed this task). "
        "docs/notes/2026-08-07-780-earth-moon-casoliva-families-gate.md + "
        "docs/notes/2026-08-08-797-casoliva-em-resonant-family-writeback.md + "
        "docs/notes/2026-08-09-801-stability-index-k-selection-fix.md. "
        "src/cyclerfinder/search/earth_moon_resonant_families.py."
    ...
    ("vaquero-21-c266-em-cycler-2013", "V1"): (
        ...
        "#828 (2026-08-12, adjudicating #822's verified free transfer, which uses THIS orbit as "
        "its Wu ORIGIN node at C=2.66): level UNCHANGED at V1, on grounds STRONGER than the bare "
        "instability clause above -- V2-ballistic (data/validation/v2_3d.py, V2_N_CYCLES_MIN=3) "
        "requires >=3 continuous bounded-drift laps of THIS IC, whereas a manifold leg is seeded "
        "x0+eps*v and integrated ~4.5-9 periods precisely so it ASYMPTOTICALLY DEPARTS, so a "
        "connection is STRUCTURALLY inapplicable to V2 quite apart from the instability; V3 needs "
        "a real ephemeris #822 never touches (pure planar CR3BP); and #822 adds nothing at V1 "
        "either, since its node re-derivation calls the SAME correct_symmetric_fixed_jacobi #799 "
        "used, as a staleness guard (same-corrector re-convergence is not an independent "
        "cross-check) and its independent-Radau is applied to the manifold LEGS' crossings, not "
        "to this orbit's own period-map closure. NO-DOWNGRADE re-confirmation (the cuts-both-ways "
        "check): #828's own from-scratch re-run reproduced #822 bit-for-bit AND the re-converged "
        "node reproduces this row's recorded state_nd to 4.4e-15, period_nd to 5.2e-14, "
        "jacobi_constant to 8.0e-15 and stability_index to 4.6e-13 relative. "
        "docs/notes/2026-08-09-799-vaquero-em-cycler-family-reproduction.md + "
        "docs/notes/2026-08-10-811-vaquero-em-cycler-family-writeback.md + "
    ...
    ("vaquero-31-c254-em-cycler-2013", "V1"): (
        ...
        "bounded-drift-over->=3-laps). "
        "#828 (2026-08-12, adjudicating #822's verified free transfer, which uses THIS orbit as "
        "its Ws DESTINATION node at C=2.54): level UNCHANGED at V1, on grounds STRONGER than the "
        "bare instability clause above -- V2-ballistic (data/validation/v2_3d.py, "
        "V2_N_CYCLES_MIN=3) requires >=3 continuous bounded-drift laps of THIS IC, whereas a "
        "manifold leg is seeded x0+eps*v and integrated ~4.5-9 periods precisely so it "
        "ASYMPTOTICALLY DEPARTS, so a connection is STRUCTURALLY inapplicable to V2 quite apart "
        "from the instability; V3 needs a real ephemeris #822 never touches (pure planar CR3BP); "
        "and #822 adds nothing at V1 either, since its node re-derivation calls the SAME "
        "correct_symmetric_fixed_jacobi #799 used, as a staleness guard (same-corrector "
        "re-convergence is not an independent cross-check) and its independent-Radau is applied "
        "to the manifold LEGS' crossings, not to this orbit's own period-map closure. "
        "NO-DOWNGRADE re-confirmation (the cuts-both-ways check): #828's own from-scratch re-run "
        "reproduced #822 bit-for-bit AND the re-converged node reproduces this row's recorded "
        "state_nd to 3.1e-15, period_nd to 1.1e-13, jacobi_constant to 5.3e-15 and "
        "stability_index to 1.4e-08 relative -- the latter measured at #799's own rtol=atol=1e-13 "
        "STM tolerance; at barden_stability's DEFAULT 1e-12 this |lambda|=11.26 orbit's half-"
        "period nu degrades to 1.3e-04 relative, an integration-tolerance sensitivity chased down "
        "rather than glossed, NOT a defect in the recorded value. "
        "docs/notes/2026-08-09-799-vaquero-em-cycler-family-reproduction.md + "
        "docs/notes/2026-08-10-811-vaquero-em-cycler-family-writeback.md + "
        "docs/notes/2026-08-12-828-vaquero-connection-tier-adjudication.md. "
        "src/cyclerfinder/search/vaquero_em_cyclers.py."
