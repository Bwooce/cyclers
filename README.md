...
**Catalogue scope (schema v5.1, expanded from v4.7 2026-06-15):** four mission-actionable orbit
classes, admitting the epoch-locked literature alongside strict cyclers, plus a fifth
corroboration-only class (`resonant_po`, schema v4.9) for stable resonant/libration orbits with
no demonstrated transport utility — carried for known-class corroboration, not as a usable
cycler.
...
392-entry published-cycler seed library (incl. the C21 3D out-of-plane `known-class-member` cycler — a computed member of the Antoniadou & Libert 2019 spatial-resonant class — Tito 2018 Mars free-return + Heaton-Longuski 2003 Uranian satellite tour U00-01 as part of the 12 `mga_tour` rows + the catalogue's **first computed `quasi_cycler` row** — Umbriel-Oberon-Umbriel (1,1) Uranian near-5:1 synodic resonance, the first-documented member of a 30-member Uranian symmetric-closure family whose 5 sibling representatives (Titania-Oberon / Ariel-Oberon / Umbriel-Titania / Ariel-Titania / Ariel-Umbriel) were added at #312-equivalent V4 — discovered + validated end-to-end by the cyclerfinder project during the 2026-06-15→06-17 frontier-capability sprint, + the **first Pluto-Charon (3,2) cycler** and the binary (k₁,k₂)-cycler μ-family reproduced from Ross-Roberts-Tsoukkas 2026, the 5 rows added 2026-06-30 (4 still V1, the Pluto-Charon representative later promoted to V2 via #505 on 2026-07-01) that closed the long-open #315/#252/#255 binary-star thread) + the catalogue's **second genuinely novel finding** — `umbriel-1-2-torus-homoclinic-uranus-2026`, a Uranus-Umbriel-Titania CCR4BP quasi-periodic torus homoclinic connection (V1, real-DE440/URA111-ephemeris-recurring at all 10 tested epochs spanning 2000–2083, independently adjudicated genuinely distinct from the Umbriel-Oberon family above — different base resonance, no shared flyby geometry), added 2026-07-24 via the `#689`→`#708` CCR4BP discovery arc (Aldrin family, Russell-Ocampo Table 3.4, McConaghy SnLm
...
resonant-family rows (2 `cycler` + 7 `resonant_po`, same-model CR3BP reproduction via `#780`'s
gate module, added 2026-08-08/09 via `#797`/`#801`). Every numerical value carries a source quote per [`data/README.md`](data/README.md)
