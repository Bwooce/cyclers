"""#393 V1 and V2 Gauntlet for Floquet low-amplitude cycler candidates.
...
Usage:
    uv run python scripts/run_393_v1_v2_gauntlet.py
"""
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-v1",
        type=Path,
        default=Path("data/branch_393_v1_verdict.jsonl"),
    )
    parser.add_argument(
        "--output-v2",
        type=Path,
        default=Path("data/branch_393_v2_verdict.jsonl"),
    )
    parser.add_argument(
        "--target",
        type=str,
    ...
    with args.input.open() as fh:
        for line in fh:
            ...
            if not line:
                continue
            row = json.loads(line)
            if row.get("kind") == "node" and row.get("node_id") == "branch_C11a_b0":
                branches.append(row)
    ...
    candidate_id = best_candidate.get("node_id")

    state0 = np.array(best_candidate["state0"], dtype=np.float64)
    period_guess = float(best_candidate["period_TU"])
    system = braik_ross_system()

    print(f"Top candidate: {candidate_id}")
    print(f"  Jacobi: {best_candidate.get('jacobi', 0.0):.5f}")
    ...
    for n in N_CYCLES_LIST:
        ...
        v = run_v2_3d(
            ...
            rtol=1e-12,
            atol=1e-12,
            notes=f"#393 Gauntlet V2 n_cycles={n} for {candidate_id}",
        )
