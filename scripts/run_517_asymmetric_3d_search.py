"""#517 Phase-A asymmetric 3D cross-system (SE<->EM) closure search using multiple cores.

Runs correct_cross_cycle_3d over asymmetric libration pairs (EM-L1 <-> SE-L2 and EM-L2 <-> SE-L1)
across a grid of out-of-plane amplitudes and Jacobi constants, utilizing parallel_sweep.
...
_NEW_ENTRY = {
    ...
    "issue": 517,
    "method": (
        "patched-CR3BP asymmetric 3D SE<->EM connection matcher + 3D bounded closure search (#517)"
    ),
    "regime": "3D Sun-Earth + Earth-Moon coupled CR3BP (patched, inertial-frame match)",
    "failed_rung": "asymmetric-closure-search-3d",
    "physical_reason": (
        "Asymmetric 3D EM->SE and SE->EM legs (connecting L1 to L2 libration regions) solved "
        "spatially to high accuracy, but the time-consistent phase residuals (R1, R2) do not "
        "close simultaneously in the grid scanned. The asymmetric configurations did not resolve "
        "the phase closure wall within this grid of single-revolution orbits."
    ),
    "resweep_condition": (
        "BCR4BP asymmetric 3D halo continuation OR a multi-revolution Metonic-commensurate 3D grid."
    ),
}
