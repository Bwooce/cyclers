"""#341 Phase 1 Part C -- 3D CR3BP existence probe at Neptune-moon systems.

Mirrors the #312 Phase 1 Part C workflow at Uranus that, combined with the
Part A wider-grid sweep + Part B other-pairs scan, surfaced the SILVER
Umbriel-Oberon-Umbriel (1,1) closure (admitted as the catalogue's first
computed quasi_cycler via #339).

Critical motivation: Triton's mean inclination relative to Neptune's
equator is ~157 deg (retrograde, deeply inclined). The patched-conic
Lambert genome used in Parts A / B is planet-frame coplanar by
construction; it never lets the spacecraft trajectory leave the moon
orbital plane. A 3D CR3BP probe (#291's correct_general_periodic_3d)
asks whether 3D periodic orbits exist AT ALL in CR3BP(Neptune, Triton)
and CR3BP(Neptune, Proteus). A clean negative is OK; a clean positive
seeds Phase 2 (3D cycler maintenance / inclination trade).

NB: the CR3BP itself is planar (in the rotating frame of the secondary
about the primary). 3D orbits here mean orbits that LEAVE the rotating
secondary-orbital plane (z != 0). This is independent of Triton's
EQUATORIAL inclination, which is a CHOICE of frame for the primary --
the CR3BP rotates with Triton, not Neptune's equator. The probe asks the
generic 3D existence question; the Triton inclination would only matter
once we re-embedded the 3D CR3BP orbit back into the Neptune-equatorial
ephemeris frame.

Seeds: x0 just inside L1, sweep z0 and ydot0 on a small log-spaced grid.
Run the full asymmetric 3D corrector with FREE_VARS_FULL_ASYMMETRIC.

NO catalogue writeback. The 3D corrector is called READ-ONLY (no
modifications to the module).

Output: ``data/scan_341_neptune_3d_probe.jsonl``.

Run as::

    uv run python scripts/scan_341_neptune_3d_probe.py
"""
...
def probe_3d_at_system(
    ...
    *,
    ...
    Identical seed convention to #312 Part C. The z0_grid is extended to
    include {5e-2, 1e-1} per the brief ("1e-3 to 1e-1 nondim").
    """
    ...
    try:
        for z0 in z0_grid:
            for ydot0 in ydot0_grid:
                ...
                rows.append(row)
                print(
                    f"[341-C] {primary}-{secondary} z0={z0:.0e} "
...
def main() -> int:
    sha = _git_sha()
    out_path = ROOT / "data" / "scan_341_neptune_3d_probe.jsonl"
    print(f"[341-C] Neptune 3D CR3BP existence probe -- sha={sha}", flush=True)
    print(f"[341-C] out={out_path}", flush=True)
    ...
    with out_path.open("w", encoding="utf-8") as fh:
        fh.write(
            json.dumps(
                {
                    "_meta": True,
                    "task": "#341 Phase 1 Part C -- Neptune 3D CR3BP existence probe",
                    "primary": "Neptune",
                    "secondaries": ["Triton", "Proteus"],
                    ...
                    "note": (
                        ...
                        "exist AT ALL in CR3BP(Neptune, Triton) and "
                        "CR3BP(Neptune, Proteus) from a small-z0 Lyapunov-"
        ...
        fh.flush()
        for secondary in ("Triton", "Proteus"):
            print(f"[341-C] Probing Neptune-{secondary} CR3BP...", flush=True)
        ...
        fh.flush()
        print(f"[341-C] DONE -- verdict: {json.dumps(verdict, indent=2)}", flush=True)
    return 0
