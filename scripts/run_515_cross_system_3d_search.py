...
# data/empty_regions.jsonl (#521 phase-1 migration) -- matching this exactly
# is what lets preflight_search()'s should_sweep() check recognise a re-run.
_REGION_ID = "cross-system-se-em-3d-patched-cr3bp-2026-07-01"
_METHOD = MethodCapability(
    genome=(
        "patched-CR3BP 3D SE<->EM connection matcher over out-of-plane amplitudes "
        "(z_em, z_se) and Jacobi constants (c_em, c_se), single-revolution"
    ),
    corrector="correct_cross_cycle_3d (bounded_ls Newton)",
    capability_tags=frozenset({"cr3bp", "patched-cr3bp", "3d", "broken-plane", "sun-earth-moon"}),
