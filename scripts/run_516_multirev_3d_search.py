"""#516 Phase-A multi-revolution 3D cross-system (SE<->EM) closure search using multiple cores.

Runs correct_cross_cycle_3d over multi-revolution grids (n_em, n_se > 1) to bypass
the phase-closure wall. Closed cycles are validated against the independent Radau integrator.
...
N_REV_PAIRS = ((1, 2), (2, 1), (2, 2))

# The region_id this search was already registered under in
# data/empty_regions.jsonl (#521 phase-1 migration).
_REGION_ID = "cross-system-se-em-3d-multirev-patched-cr3bp-2026-07-01"
_METHOD = MethodCapability(
    genome=(
        "patched-CR3BP multi-revolution 3D SE<->EM connection matcher over "
        "(n_em, n_se) revolution-count pairs > 1, out-of-plane amplitudes and "
        "Jacobi constants"
    ),
...
_NEW_ENTRY_ID = "cross_system_se_em_3d_multirev_patched_cr3bp"

_NEW_ENTRY = {
    "id": _NEW_ENTRY_ID,
    "issue": 516,
    "method": (
        "patched-CR3BP multi-revolution 3D SE<->EM connection matcher + "
        "3D bounded closure search (#516)"
    ),
    "regime": "3D Sun-Earth + Earth-Moon coupled CR3BP (patched, inertial-frame match)",
    "failed_rung": "multirev-closure-search-3d",
    "physical_reason": (
        "Multi-revolution 3D EM->SE and SE->EM legs solved spatially to "
        "high accuracy, but the time-consistent phase residuals (R1, R2) "
        "do not close simultaneously in the grid scanned. The additional "
        "revolution knobs (n_em, n_se > 1) did not resolve the phase closure "
        "wall within this grid of 3D orbits."
    ),
    "resweep_condition": (
        "BCR4BP 3D halo continuation OR a multi-revolution Metonic-commensurate "
        "3D grid with longer integration horizons."
    ),
}
...
def main() -> None:
    print(f"[{_ts()}] #516 multi-revolution 3D cross-system closure search starting.")
