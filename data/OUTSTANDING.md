...
- **Two more instances of the same pattern, found and corrected during this pass:** (1) `#589`'s
  ...
  **UPDATE 2026-07-17 (`#623` pass): all four named one-liners (`#582`/`#583`/`#585`/`#589`) have
  now been hand-corrected in place in the ledger paragraph** (each marked "one-liner corrected
  2026-07-17" inline). No claim is made that these four were the only stale ones — the
  rough-index-only caution stands for the rest of that paragraph.
...
this family, a re-grounded grid still finds the two manifolds never co-reach the patch section
...
- **#615 ✓ DONE (2026-07-16) — BOTH NEW HYPOTHESES DECISIVE NEGATIVES: neither the free-rho
  fixed-C=3.15 branch nor the NRHO orbit-type reproduces O&B's 0.2739/0.02163 pair. The
  `#548`->`#613` Owen & Baresi non-reproduction is now exhausted on all four fronts (pin, `C`,
  free-rho, orbit-type).** `#613` follow-up, motivated by reading the source PDF directly.
  **Two untested hypotheses (from the paper, not the digest):**
  **(1) §2.3 (p.4): at FIXED Jacobi constant, quasi-halo tori form a 1-PARAMETER family in
  rotation number** — so `#612`'s hard rho-pin may have hidden a large-amplitude branch that
  drifts to 0.2739. **(2) Fig. 5 (p.6) shows an L2 southern NRHO** — the §4.1.1 orbits might be
  large-amplitude NRHOs, not the near-bifurcation halos every prior task assumed.
  **Paper re-read (full 19pp) — nothing that changes the outcome, but confirmed:** §4.1 μ=0.012153643,
  **all** EM connections at Jacobi integral C=3.15; §4.1.1 gives the quasi-halo pair L1=0.2739 /
  L2=0.02163 "latitudinal frequencies" (θ1 = latitudinal per §2.3). **The paper gives NO orbit ICs,
  periods, or any table**, and §4.1.1 never states the orbit type — the NRHO is only an illustrative
  regularisation figure (Fig. 5). (Aside: the codebase's ratio interpretation of "latitudinal
  frequency" = ω_trans/ω_long is validated by L2 — its near-bifurcation ratio 0.0198 sits right at
  O&B's 0.02163; kept that reading.)
  **H1 — free-rho, fixed-C=3.15 pseudo-arclength continuation (the headline build). DECISIVE
  NEGATIVE.** New `continue_qp_torus_fixed_jacobi` walks the family in the full pseudospectral
  solution space `(coeffs, ω1, ω2)` with rho NOT pinned: it REUSES `#612`'s invariance-PDE residual
  + analytic Jacobian UNCHANGED (with all gauge rows zeroed) and appends its own 2 phase-gauge +
  1 Jacobi-constraint + 1 pseudo-arclength rows. The arclength constraint (not a rho pin) is what
  suppresses the degenerate ω2->0 collapse — that branch has a different local tangent. Key design
  fix found the hard way: the 4 constraint rows must be heavily weighted (~1e3) or the ~1500 PDE
  rows outvote the single Jacobi row and the solver drifts off-energy / collapses (an early run
  leaked to C=1.575, exactly half — a Jacobi-convention bug: codebase C = 2Ω−v², not the paper's
  Ω−v²/2). **Result:** starting from the parent L1 halo (amp≈0.0015, rot=−0.0748), as transverse
  amplitude grows to a FOLD at ~0.09–0.11, the free rotation number |rho| MONOTONICALLY DECREASES
  from 0.0748 to ~0.057 — i.e. it moves AWAY from 0.2739, never toward it; C is held at 3.150000
  (±1e-4) and the PDE residual stays <1e-5 the whole way. The entire fixed-C=3.15 family's rho lives
  in ~[0.057, 0.075], a factor >3.6 below the target. So the paper's 1-parameter family IS real
  (rho genuinely varies — this REFINES `#612`'s "energy-pinned flat" claim, which was flat only
  because it pinned rho) but its range at C=3.15 simply does not contain 0.2739.
  **H2 — NRHO orbit-type hypothesis. DECISIVE NEGATIVE, and physically excluded.** New
  `halo_family_rotation_profile` marches the L1/L2 halo families into the deep large-amplitude /
  NRHO regime recording each member's Neimark-Sacker rotation number. **The halo family's Jacobi
  constant is CAPPED at its planar-Lyapunov bifurcation** (L1 measured max 3.15004, L2 3.15000),
  reached at ~zero amplitude; the large-amplitude members drop to lower C (L1 reaches z0=−0.243 /
  C=2.984, genuine NRHO territory, at C≈2.98–3.02). **So no NRHO exists at C=3.15 at all** — O&B's
  C=3.15 quasi-halos are necessarily the small-amplitude near-bifurcation halos, the orbit type
  every prior task used. Independently: along the family the rotation number never realises the
  (0.2739, 0.02163) pair at any single C — L1's rot rises to a max ~0.498 at C≈3.02 then turns over
  (crossing 0.2739 near C≈3.05, echoing `#613`'s C≈3.076); L2's rot is MINIMISED (0.0198) at its own
  C=3.15 seed and only rises (to >0.26) with amplitude.
  **CONCLUSION (final): the `#548`->`#553`->`#555`->`#556`->`#612`->`#613`->`#615` Owen & Baresi
  chain is exhausted.** Four independent framings all fail to build the isoenergetic L1<->L2
  quasi-halo pair O&B describe at C=3.15: rho-pinned amplitude (`#612`), rho-vs-C (`#613`), free-rho
  fixed-C (H1), and NRHO orbit-type (H2). L1's 0.2739 remains individually reachable only at C≈3.05–3.08
  (`#613`), where L2 is nowhere near 0.02163. The most likely residual explanation for the paper's
  numbers — untestable here without the authors' ICs — is a differing torus parameterisation /
  ...
  **New module** `src/cyclerfinder/search/qp_torus_fixed_jacobi_continuation.py`
  ...
  (no bug found in any). **Tests:** `tests/search/test_qp_torus_fixed_jacobi_continuation.py`, 4
  live-pinned tests, all pass; ruff clean. Sibling suites `test_variational_qp_torus.py` and
  `test_halo_family_at_jacobi.py` re-run fully green (20/20 across the three files). No
  `scripts/run_*.py` created (all exploration scratch-only), so the preflight AST ratchet does not
  apply. Left uncommitted for coordinating-session review.
  **Recommended model (if ever revisited): Opus** — but the honest recommendation is that this chain
  is closed; further O&B work should wait on obtaining the authors' actual orbit ICs / frequency
  convention, which is an input gap, not an iteration gap.
...
- **#621 ✓ CLOSED 2026-07-17, CLEAN NEGATIVE (0/4 papers golden-worthy)** — the "expand `#267`
  resonance_network goldens" mining step `#503`
  left open. `src/cyclerfinder/search/resonance_network.py`'s `_RESONANT_SEEDS` table currently has
  its `"*-Kumar"` entries sourced from Kumar-Rawat-Rosengren-Ross's own Table 6 (fixed by `#598`,
  which is the model to follow here: find each paper's own tabulated converged-orbit initial
  conditions, cross-check against the paper's own text/figures before trusting a table transcription,
  add as a new sourced `_RESONANT_SEEDS` entry + a corresponding golden test in
  `tests/search/test_resonance_network.py` that reproduces the paper's own stated period/energy to a
  documented tolerance). **Scope: re-read the 4 already-acquired-and-digested `#503` papers'
  DIGESTS FIRST** (`docs/notes/2026-06-30-digest-rawat2026-cislunar-mmr.md`, `-onozaki2017-tube-4body.md`,
  ...
  C1.9.5's "4:1 isolated by an RIC barrier" is a topological claim, not a numeric golden) — a
  well-characterized "this paper has nothing golden-worthy, here's why" for any of the 4 is a
  completely acceptable, honest partial outcome; do not force an extraction that isn't really there.
  Follow `#598`'s own sourced-golden discipline exactly: expected test values must trace to a
  publication table/figure, never to this codebase's own computed output
  (`[[feedback_golden_tests_sourced_only]]`). **Recommended model: Sonnet** (digest-verification +
  ...
  **OUTCOME: 0 new `_RESONANT_SEEDS` entries — all 4 papers checked and ruled out against the source
  ...
  WIDTHS in km and aggregate spacecraft Jacobi statistics (Section VII, Figs. 8–11), not a table of
  individual converged periodic-orbit Cartesian ICs — verified directly against the source PDF's
  Section V/VII text and figures, which describe the unstable-orbit continuation *method* but never
  tabulate specific `(x0, ydot0)` states the way Kumar's Table 6 does. (2) `onozaki2017-tube-4body`
  has genuinely precise numeric ICs (departure/patch/arrival states, Section 5.3), but they are
  points on a bicircular-restricted-4-body (BCR4BP) transfer trajectory in the Sun-Earth-Moon system
  — a different, non-autonomous dynamical model than the autonomous 2-body-mass CR3BP this module's
  `correct_symmetric_fixed_jacobi` corrector implements, and not periodic orbits at all (confirmed
  from the source PDF's own "planar bicircular restricted 4-body system" framing). (3)
  `fitzgerald2022-transit-perturbed-rtbp` has real periodic-orbit ICs (Appendix A, verified verbatim
  against the source PDF: BCP L1 `[0.837595408485656, 0, 0, 0.827678389393936]`, ER3BP L1
  `[0.792718947200736, 0, 0.000001145970495, 0.886145419995798]`) — but these are Lagrange-point
  replacement periodic orbits in the time-periodic BCP/ER3BP models (confirmed via the source PDF's
  own Fig. 1/Fig. 2, which explicitly contrasts CR3BP's equilibrium point against BCP/ER3BP's
  periodic orbit), not the autonomous CR3BP this module models; the paper's own digest already routes
  this data to `#292`/`#293` instead. (4) `naik2017-lobe-dynamics-transport` is a pure
  computational-methods paper (the "Lober" lobe-area software) whose only worked numeric examples are
  a fluid-dynamics oscillating-vortex-pair flow and a ship-capsize 2-DOF system (confirmed via the
  source PDF's abstract/intro) — no CR3BP orbits of any kind. No changes made to
  `resonance_network.py` or `tests/search/test_resonance_network.py` (still 13 tests, unchanged);
  `uv run ruff check . && ruff format --check .` green; full `tests/data tests/search -q` ratchet
  green. This closes the `#503` follow-up thread.
...
- **#617** (dispatched 2026-07-16, user-approved revisit of `#538`/`#544`) — build a seedless
  QBCP quasi-periodic TORUS corrector, the exact missing piece `#544`'s deepest diagnosis
  identified as the real fix for the EM-L2 torus that has blocked `#538`'s cislunar cycler closure
  since 2026-07-09: `genome/qbcp_torus.correct_qbcp_torus` runs single-period GMOS (shooting over a
  full `T_s` per invariant-circle sample), which is fine for the mildly-unstable SE-L2 torus
  (residual 3e-5) but hopeless for the violently-unstable EM-L1/L2 region (each sample amplified
  ~1e6-1e8) — `#544`'s own words: "The real fix is a multiple-shooting GMOS corrector... NOT more
  Fourier modes, a different target orbit, or coefficient hunting." **This is EXACTLY the wall
  `#611` already crossed for the QBCP PERIODIC ORBIT case** (`src/cyclerfinder/search/
  variational_periodic_orbit_qbcp.py` — seedless, no integration in the search, crosses the
  identical EM-L1/L2 instability to machine precision) **and `#612` already crossed for the CR3BP
  quasi-periodic TORUS case** (`src/cyclerfinder/search/variational_qp_torus.py` — 2D pseudospectral,
  free frequencies, analytic Jacobian, no integration). Nobody has combined the two: a QBCP
  (non-autonomous, fixed-period) quasi-periodic (2-angle) torus corrector. Given the strong track
  record of both precedents, this is a high-confidence-of-success capability build, not speculative
  exploration — but per this project's own portfolio-parking note (this whole corridor absorbed
  5+ tasks across sessions with zero catalogue rows before being deprioritized on 2026-07-10 for low
  novelty payoff, NOT technical difficulty), **explicitly do NOT proceed into re-running `#538`'s
  full cross-system SE<->EM boundary-value correction chain** once this capability lands — that
  payoff/investment decision is separate and belongs to the coordinating session/user, not this
  task. **Scope: build + validate only.** (1) Adapt `#612`'s 2D pseudospectral torus machinery to
  the QBCP's non-autonomous, fixed-period setting the way `#611` adapted the periodic-orbit case
  (fixed period = `period_multiple * T_s`, no phase-gauge-via-free-period since there's no
  continuous time-shift symmetry — reuse `#611`'s own reasoning here, it's the identical
  adaptation). (2) Positive control: reproduce the SE-L2 torus that already converges cleanly under
  the existing GMOS method (residual ~3e-5) as the regression floor. (3) Headline target: the EM-L2
  torus at whichever Jacobi constant `#544`'s investigation left as the best candidate (re-check
  `#544`'s own bullet for the exact value/history — it tried both 3.17 and 3.13, with 3.13
  converging better on the BCR4BP side) — report the ACTUAL achieved invariance residual, honestly,
  whether it crosses `#544`'s originally-targeted 1e-3 gate or not. A genuine wall-crossing here
  would be a significant, citable capability result on its own (independent of whether `#538`'s
  larger cross-system goal is ever pursued further); a negative would be equally informative given
...
- **#619 ✓ CLOSED, CLEAN NEGATIVE (2026-07-17) — the `#538`-`#619` arc's FINAL outcome: no closed
  ...
  **This is NOT a simple torus swap** — `scripts/run_538_qbcp_cycler.py`'s existing connection-search
  architecture (`build_tori`, `manifold_state_vec`, `_scan_crossings`, `build_seed`,
  `make_full_residual`, `_multistart_ballistic`) needs STABLE/UNSTABLE MANIFOLD directions off each
  torus (Floquet-type eigenvectors), which `#617`/`#618`'s pseudospectral corrector does not compute
  (it only solves for the invariant torus itself). **Good news: the mechanism to get this already
  exists and is reusable.** `manifold_state_vec` (line ~319) extracts stable/unstable eigenvectors by
  propagating the AUGMENTED STM equations (`qbcp.propagate_qbcp_pv(..., with_stm=True)`) for exactly
  ONE stroboscopic period `t_strob` from an ALREADY-CONVERGED torus point, then calling
  `local_stability()` — this is legitimate, NOT the shooting-based Newton-iteration fragility `#544`
  root-caused (that was about repeated single-period shooting evaluations from an IMPERFECT guess
  during the search itself; a ONE-SHOT propagation of a genuinely near-invariant converged point to
  extract its own linear stability is standard practice for any validated periodic/quasi-periodic
  solution, same category as `#617`/`#612`'s own independent closure checks). **Scope:**
  (1) Build a thin adapter giving `#618`'s `QBCPTorusVariationalResult` the same interface
  `manifold_state_vec` expects (`evaluate_torus_state` → PM state → PV via `state_pm_to_pv`,
  `omega1`/`period` in place of `omega_long`/`t_strob`), reusing `manifold_state_vec`'s/
  `local_stability`'s logic unchanged where possible. (2) **Positive control (mandatory):** validate
  the extracted stable/unstable eigenvector DIRECTIONS against the existing GMOS SE-L2 torus's
  already-trusted manifold data — do the two methods agree? (3) **Honest check before trusting
  anything downstream:** `#618`'s EM-L2 torus has residual ~9.5e-4, not machine precision — verify
  empirically (don't assume) whether propagating from a not-quite-exact torus point for one
  VIOLENTLY-UNSTABLE period corrupts the extracted eigenvector DIRECTIONS (which may be far better-
  conditioned than the raw trajectory itself, since they come from the linearized dynamics' spectral
  decomposition — but this must be checked, not assumed, per this project's "check, don't guess"
  discipline). (4) Feed the new EM-L2 manifold data (paired with SE-L2's existing GMOS data) into
  `#538`'s EXISTING multi-segment corrector (Task 2 of `docs/superpowers/plans/2026-07-09-538-qbcp-
  cross-system-cycler-correction.md`: 12-unknown/18-residual, position+velocity+periodicity+phase-
  return, ballistic-first) and run it. (5) Powered fallback (the plan's Task 3) only if ballistic
  genuinely fails to converge in-basin. (6) **MANDATORY independent-integrator cross-check** (the
  plan's Task 4, Radau vs. DOP853, re-derived periodicity from raw propagated states, not the
  optimizer's own residual) before calling ANYTHING "closed" — per this project's orbit-closure
  discipline, "it closed!" is the danger signal, not the finish line. **A clean, well-characterized
  negative — genuinely does not converge, or converges but the cross-check falsifies it — is a fully
  acceptable, valuable, FINAL outcome for this whole `#538`-`#619` arc; do not force a "closed"
  result.** **Explicit writeback gate: even if genuinely confirmed closed, do NOT write to
  `data/catalogue.yaml` yourself.** Hold at the report stage — this needs an independent Fable
  adversarial review pass (matching the plan's own Step 3a discipline and this project's established
  practice for high-stakes closure claims) plus the coordinating session's own independent numerical
  verification, both still to come, before any writeback. Long individual propagation/solve steps
  are expected and fine (`#618`'s single solves took 13-22 minutes) — this task may run long; that is
...
- **#620 ✓ CLOSED, CLEAN NEGATIVE (2026-07-17) — a NEW obstruction (ghost minima), distinct from
  `#619`'s manifold conditioning; closes the last untried alternative for the `#538`-`#620` arc.**
  (dispatched 2026-07-17, user-approved new research direction, `#619` follow-up) — a
  genuinely different attack on the cross-system SE<->EM QBCP connection, avoiding the manifold-
  conditioning wall `#619` found entirely rather than trying to work around it. Every seedless
  corrector this session (`#611`/`#612`/`#617`/`#618`) succeeded by the SAME principle: represent the
  unknown object spectrally and drive an algebraic residual to zero via collocation, with NO forward
  integration (and hence no per-period error amplification) anywhere in the search. `#538`'s
  connection-finding approach still depends on a linearized manifold DIRECTION to seed a shooting
  search — exactly the ingredient `#619` showed is unrecoverable at the EM-L2 region's achievable
  precision. **The idea: extend the same collocation philosophy to the CONNECTION ITSELF.** Represent
  BOTH transfer arcs (the `#538` plan's segments 1 and 3, SE-L2->EM-L2 and EM-L2->SE-L2) as truncated
  Chebyshev or Fourier collocation trajectories in a normalized time variable over their own (free)
  duration, driving the `qbcp_eom` ODE residual to zero pointwise at collocation nodes (exactly the
  `#611`/`#617` no-integration pattern, generalized from a periodic/quasi-periodic BOUNDARY object to
  an open ARC), with BOTH endpoints matched (position+velocity continuity) to points on the
  ALREADY-CONVERGED SE-L2 (existing GMOS) and EM-L2 (`#618`'s n1=28,n2=9 pseudospectral) tori — the
  torus phase angles at each endpoint stay free unknowns (reusing `#538`'s own Task-1 residual-shape
  philosophy), but NO manifold-perturbation seeding or shooting is used to build the arc at all. One
  combined least-squares residual: arc-1 collocation defects + arc-2 collocation defects + both
  endpoint-matching conditions + the synodic-periodicity/phase-return condition. **This is genuinely
  new, uncertain research — not guaranteed to work.** The open question this task must answer
  empirically: does a violently-unstable TRANSFER ARC (not just a boundary torus) actually behave
  better under collocation than shooting, or does its own high local curvature demand an infeasible
  polynomial order (mirroring why `#618` needed `n1,n2` up to 28,9 just for the boundary torus)? **A
  clean negative — collocation needs an infeasible node count, or the combined residual has no
  accessible basin, or some other genuine obstruction — is a fully acceptable, valuable, FINAL answer
  to this specific idea, closing the loop on the one alternative `#619` itself flagged as untried.**
  **Scope: build + validate + attempt, same discipline as `#611`-`#619`.** (1) Design the arc
  collocation representation and residual (the crux judgment call — take real design time, document
  reasoning thoroughly). (2) Positive control: first validate the arc-collocation machinery on a
  BENIGN case before the violently-unstable one (e.g. a short arc between two nearby points on the
  SAME already-converged SE-L2 torus, or reproducing some other already-known, well-behaved QBCP
  trajectory segment) — machinery must be proven sound before the hard case means anything. (3)
  Attempt the actual SE-L2<->EM-L2 connection. (4) **Mandatory independent cross-check** (Radau vs.
  whatever integrator the collocation itself implicitly assumes, re-derive periodicity from raw
  propagated states, not the optimizer's own residual) before calling ANYTHING closed — same
  orbit-closure discipline as every other task this session. **Explicit writeback gate: do NOT write
  to `data/catalogue.yaml` even on an apparent success.** Hold for an independent Fable adversarial
  review plus the coordinating session's own separate verification, exactly as `#619` was gated.
  **Recommended model: Opus** (the collocation-representation design is the single hardest judgment
  call in this whole session's arc — same tier as `#611`/`#612`/`#617`).
  **✓ DONE (2026-07-17) — BUILT + POSITIVE-CONTROLLED + HARD CASE ATTEMPTED: a CLEAN, WELL-
  CHARACTERIZED NEGATIVE with a NEW, specific obstruction distinct from `#619`'s. No closed cislunar
  cycler found; the one alternative `#619` flagged as untried is now closed.** New module
  `src/cyclerfinder/search/variational_qbcp_arc.py` + tests `tests/search/test_variational_qbcp_arc.py`
  (12 tests, all green; adjacent `test_variational_qbcp_torus.py` + `test_variational_periodic_orbit_qbcp.py`
  re-run green, 42 total). NO existing module modified. No `scripts/run_*.py` created (library module +
  test only, so the preflight AST ratchet does not apply). No catalogue writeback (there is no closure to
  write, and the `#620` gate holds regardless). Left uncommitted for coordinating-session review.
  **Design (Step 1, the crux).** Each transfer arc is an OPEN trajectory over a free duration `tau`,
  represented as a Chebyshev-Gauss-Lobatto pseudospectral series in the 6 canonical (PM) state
  components over `s in [-1,1]`, `t(s)=t_start+(tau/2)(s+1)` (CHEBYSHEV for the open arc, vs the FOURIER
  basis `#611`/`#617` use for the closed periodic/quasi-periodic boundary objects). The ODE residual
  `(2/tau)(D u)_j - qbcp_eom(t_j,u_j)=0` is driven to zero at the INTERIOR CGL nodes (endpoints pinned
  by the match rows — the standard crisp pseudospectral BVP; imposing the ODE at ALL nodes instead
  floors even a true trajectory at the ~1e-2 polynomial-interpolation error, measured directly). NO
  integration in the search — the `#611`/`#617` "no per-period amplification in the residual" principle,
  lifted from the boundary object to the connecting arc, so `#619`'s unstable-manifold-direction
  ingredient is NEVER computed. Both arc endpoints match (full position+VELOCITY, in PV) points on the
  two ALREADY-CONVERGED tori (existing SE-L2 GMOS, invariance 1.538e-5; `#618`'s EM-L2 pseudospectral
  n1=28,n2=9, rebuilt live to the identical residual_rms 9.4744e-4 / rot 1.85575 / amp 0.17881 in 739s),
  with the four torus phase pairs FREE (reusing `#538`'s Task-1 residual-shape philosophy) plus epoch
  congruence + SE/EM phase closure. Combined two-arc residual solved ONCE via `least_squares(method="trf")`.
  Jacobian: the large well-conditioned node-value block is EXACT/analytic (`(2/tau)D⊗I6 - blockdiag(A)`
  with the `qbcp_stm_eom` state-Jacobian, plus the exact PM→PV map for the match rows), the ten
  phase/duration columns finite-differenced internally (well-justified: the SE GMOS `evaluate_qbcp_torus`
  PROPAGATES to place a point, so `d(point)/d(theta)` has no closed form) — node block verified vs central
  FD to rel < 1e-7.
  **Two positive controls PASSED (Step 2).** (a) Full two-arc corrector on a genuine `#611` QBCP periodic
  orbit SPLIT into two arcs (endpoints as fixed physical states): combined residual to MACHINE precision
  (3.8e-15), all match/time/phase-closure rows ~1e-14, independent Radau loop defect 1.8e-4 (limited only
  by the input orbit's own ~3e-6 closure, not the corrector) — validates the entire residual + analytic-
  Jacobian + independent-check path against a KNOWN closed trajectory. (b) A short collocation arc between
  two real SE-L2 GMOS torus points reproduces the on-torus QBCP segment (residual machine-zero, nodes
  match a direct integration to 1e-6, independent Radau arrival < 1 km). Machinery proven sound.
  **DECISIVE MID-BUILD FINDING (mandatory-check discipline made concrete): the algebraic collocation
  residual is SATURATED with spurious "ghost" minima.** In the violently-unstable regime a degree-K
  polynomial can hit MACHINE-ZERO nodal residual while NOT being a real trajectory — the interpolant
  satisfies the ODE exactly at the K-1 interior nodes but diverges wildly between them. Measured directly:
  from a poor (linear-interpolation) seed the periodic-split corrector reaches residual 8.7e-15 with the
  endpoints hitting the target points (~1e-13) yet the independent Radau loop defect is 1.5 (O(1), ~20,000
  km arrival error), `converged=False`. The independent Radau re-propagation cleanly separates real
  connections (loop ~1e-12) from ghosts (loop O(1)) and is the SOLE arbiter — exactly this project's "it
  closed is the danger signal." Both this ghost phenomenon and the machinery-correctness are pinned as
  tests.
  **HARD CASE (Steps 3+4): SE-L2 ↔ EM-L2 connection — NO Radau-confirmed connection found across 24
  diverse attempts.** Fed the converged SE-L2 GMOS and `#618` EM-L2 (n1=28,n2=9) tori as the two endpoint
  objects, phases free, seeded by linear PM interpolation (the honest NO-manifold seed) over a broad
  multi-start: orders 30/40/60 × 8 phase/duration seeds each (tau in {2,3,5,8, 0.5·T_s, T_s, 1.5·T_s}).
  Every attempt landed in ONE of two non-solutions: (i) a GHOST (algebraic residual → 1e-13..1e-14, endpoints
  matched to ~1e-13, but Radau loop defect ~3–5 nondim, arrival error ~1–1.8 MILLION km) — 3/24, more common
  at lower order where the polynomial has just enough slack to interpolate a wild path; or (ii) a FLOOR
  (residual stalls at ~0.006–0.45, endpoint match_f O(1), so it can't even zero the algebraic residual)
  — 21/24, dominant at order 60 where the higher node count leaves no room for a clean ghost. The BEST
  independent-Radau loop defect across ALL 24 was 1.893 nondim (order 40; arrival gap 351,467 km ≈ one
  Earth–Moon distance) — nothing came within orders of magnitude of a real connection. **Independent
  cross-check (Step 4) is built into every solve** (`independent_closure_check`, Radau re-propagation of the
  raw departure node, independent of the collocation's implicit polynomial): it confirms there is no
  spurious "it closed" to guard against — the corrector simply, honestly, does not close.
  **Verdict — a NEW obstruction, distinct from `#619`'s, and a fitting FINAL close of the untried
  alternative.** The collocation approach DOES successfully avoid `#619`'s wall (it never computes an
  unstable-manifold direction, and ran without difficulty). But it hits its OWN, different fundamental
  obstruction: in this violently-unstable region the integration-free algebraic residual is degenerate as
  a SEARCH objective — its zero-set is saturated with ghost polynomials indistinguishable (by the residual)
  from real trajectories, so from any seed not already in the narrow basin of a genuine connecting trajectory
  the solve is drawn to a ghost (or, at high order, floors). Collocation here can VERIFY a near-real seed
  (both positive controls prove this cleanly) but cannot SEARCH for the connection without one — and a
  near-real cross-system seed is exactly what does not exist (it is what we are looking for, and `#619`'s
  best manifold-shooting attempt only reached ~166,000 km, itself not a real connection). Two
  methodologically-INDEPENDENT approaches — `#619`'s linearized manifold shooting and `#620`'s
  integration-free global collocation — now AGREE in finding no closed SE-L2 ↔ EM-L2 cislunar cycler
  (best gaps 351,467 km here vs ~166,000 km there, both integrator-confirmed real, both O(10^5 km)). This
  is NOT a proof that no such cycler exists (not claimed), but it closes the one untried alternative and is
  a clean, valuable, FINAL negative for the whole `#538`-`#544`-`#611`-`#612`-`#617`-`#618`-`#619`-`#620`
  arc. Possible (uncertain) future refinement, NOT done here: seed the collocation from `#619`'s best
  ~166,000 km ballistic near-miss to test whether global collocation can REFINE a shooting near-miss into a
  real connection (would require reconstructing that trajectory via the manifold machinery `#620` deliberately
  avoids), and/or an over-collocation (m ≫ order least-squares) variant to suppress ghosts at the cost of the
  interpolation-error floor. **Writeback gate honored: NO `data/catalogue.yaml` write (there is no closure,
  and the gate holds regardless); held for the coordinating session.**
...
- **#411 ✓ EM-L2/SE-L2 route CHARACTERIZED NEGATIVE (2026-06-20) — header corrected
  2026-07-17, was stale "IN PROGRESS."** Cross-system closure pursuit, post-bugfix.
  ...
  (time-consistent 2×2 Newton over c_em,c_se). The Newton STALLS at |R|≈0.59 rad —
  both legs stay converged and cheap throughout, but R2 floors nonzero (c_se steps
  fall off the finicky SE family near the Canalias bifurcation). **A same-day follow-up
  note this bullet never absorbed** (`docs/notes/2026-06-20-411-corrector-outcome.md`,
  commit `452b28d`) then tested rev-count variation directly (n_se=2: floor shifts
  0.59→0.78 rad, does not reach zero) — confirming a **1-DOF obstruction**: the
  both-legs-converge spatial constraint couples (c_em, c_se) into a 1-D curve, leaving
  only one phase-consistency residual zeroable, not two. **Verdict: the EM-L2/SE-L2
  patched-CR3BP route is a fully characterized negative, not a stall awaiting a
  solver fix** — both legs stay ballistic-cheap (~0.8 km/s total) throughout, so the
  obstruction is phase-closure dimensionality, not energy. Two untested venues were
  named: (1) other libration pairs — 2 of 3 (EM-L1↔SE-L2, EM-L2↔SE-L1) were
  subsequently swept by `#517` (2026-07-01, `data/empty_regions.jsonl` entry
  `cross-system-se-em-3d-asymmetric-patched-cr3bp-2026-07-01`, 0/48 closed), leaving
  only **EM-L1↔SE-L1 genuinely untested** — see `#622` below; (2) BCR4BP with an
  SE-scale seed (`#412`'s re-scope) — a bigger from-scratch build, not attempted.
...
- **#316** Cross-system cycler framework (Sun-Earth ↔ Earth-Moon manifolds) — **user decision
  ...
  obstruction) worth its own continued resolution rather than folding into that arc's conclusion —
  see `#622` below for the revival (EM-L1↔SE-L1, the one libration pair `#411`'s own next-venues
  list named that remains genuinely untested after `#517`).
- **#622 ✓ CLOSED, CLEAN NEGATIVE (2026-07-17)** — the EM-L1↔SE-L1
  cross-system pair is the one cell of the "4 libration pairs" matrix (`#411`'s EM-L2/SE-L2,
  `#517`'s EM-L1↔SE-L2 and EM-L2↔SE-L1, and this one) that has never actually been swept: `#520`
  aimed to cover all 4 in one 8,640-point grid but ABORTED after 12+ hours with zero output (no
  checkpointing, too coarse + too expensive — see `#520`'s own bullet). Scope: reuse the existing,
  already-tested corrector infrastructure — `genome/cross_system_cycle.py`'s `correct_cross_cycle`
  (planar, `#411`) and/or `correct_cross_cycle_3d` (3D, `#515`) — and run JUST the EM-L1↔SE-L1
  pair, at single AND low-multi-revolution (n_em, n_se ∈ {1,2,3}, per `#411`'s own "cheapest next
  try" note), following `#516`/`#517`'s scope/format (a narrow, single-pair sweep, NOT `#520`'s
  full matrix) so this doesn't repeat `#520`'s mistake.
  **Feasibility scan first** (per `[[project_negative_results_registry]]`-style pre-check): the
  `#516`/`#517` (c_em, c_se) grid, calibrated around the Canalias SE-L2 bifurcation
  (3.0003/3.000863625), turned out to be WRONG for this pair — at those C values the SE-L1
  stable manifold's own radial reach from Earth stays under ~0.9e6 km, well short of the
  1.5e6 km default patch section (that near-bifurcation neutral-stability boost is an SE-L2-only
  effect). A standalone manifold-reach probe found a re-grounded range (c_em ∈ [3.12, 3.14],
  c_se ∈ [2.9998, 3.0000]) where EACH manifold's own peak radial reach individually clears
  1.5e6 km. Ran `scripts/run_622_em_l1_se_l1_search.py` (new file) on that re-grounded grid ×
  n_em, n_se ∈ {1,2,3}² = 36 points (timing pilot: single cheapest point 7.06s; full sweep
  283.8s ≈ 4.7 min, run twice for reproducibility — bit-identical both times). **Result: 0/36
  closed, 0/36 even reach FORWARD-leg spatial convergence, 0/36 return-leg convergence.** Even
  though each manifold's peak radial reach individually clears the patch distance, the EM-L1
  unstable manifold and SE-L1 stable manifold never CO-reach the inertial patch section at any
  sampled (θ, τ_u, τ_s) — confirmed both at the corrector's default scan resolution and at 4×
  that resolution in a standalone check. This is a more fundamental failure than `#411`'s
  EM-L2/SE-L2 result: that pair achieved Task-3 spatial closure cheaply (~0.8 km/s) and stalled
  only on phase time-consistency; EM-L1/SE-L1 never reaches spatial closure at all, so
  revolution count (n_em, n_se) is moot — it cannot rescue a connection that doesn't spatially
  exist. Completes the 4-libration-pair matrix as a characterized negative. Registered as
  `region_id=cross-system-se-em-l1l1-patched-cr3bp-2026-07-17` in `data/empty_regions.jsonl`
  (NOT `data/negative_results.yaml`, which is FROZEN as of the `#521` phase-1 migration — its
  own header says new negatives go directly to `empty_regions.jsonl` via `append_empty_region()`;
  `#515`-`#517`'s scripts predate that freeze and are not a template for the registration step,
  only for the sweep-scope/instrumentation convention). Per-point progress + incremental JSONL
  checkpointing to `data/runlogs/622_em_l1_se_l1_checkpoint.jsonl` (gitignored) throughout, per
  `[[feedback_incremental_progress_reports]]` — no `#520`-style silent-hours-then-nothing risk.
  No catalogue writeback (nothing closed). `uv run ruff check .`/`ruff format --check .` clean;
  `tests/search tests/data tests/scripts` green (no new reusable library code factored out, so
  no new unit tests — the AST preflight ratchet in `tests/scripts/test_scripts_call_preflight.py`
  already covers the new script).
- **#623** (dispatched 2026-07-17, user-requested) — a `#605`-style Fable strategic-review pass,
  but this time reviewing (a) the reconciled `## CURRENT STATE` dashboard's genuinely-open items
  and their relative priority, and (b) the much larger capability set built across the
  `#605`-`#622` arc for genuinely novel COMBINATIONS not yet tried and any new literature worth
  pulling in. Analysis/planning only — no code, no catalogue writeback, no new search dispatch;
  the durable record is this bullet (updated with the outcome) plus any docs/notes/ doc the
  reviewing agent chooses to write.
  **✓ DONE (2026-07-17) — full report at `docs/notes/2026-07-17-623-strategic-review.md`.**
  **(a) Priority read:** `#542` — keep open for exactly ONE more bounded pilot (cross-μ transfer
  test of `#608`'s already-built model at μ=0.001/Sun-Earth μ, ~hours, `#608`'s own evaluation
  protocol), then close it either way — after `#614`'s double negative, model sophistication is
  not the binding constraint and the "unfamiliar basin" claim is now a single falsifiable
  transfer question. `#520` — do NOT revive, disagreeing with the standing timed-pilot-redesign
  framing: `#622` (running) completes the 4-pair matrix's last cell, after which `#520`'s only
  remaining content is "same 4 pairs, denser grid" — the exact anti-pattern `#411` warns against,
  with the QBCP-fidelity version of the same question already a two-method negative
  (`#619`/`#620`); mark `#520` CLOSED/SUPERSEDED once `#622` lands. Ledger-staleness housekeeping —
  DONE in this pass: all four flagged one-liners (`#582`/`#583`/`#585`/`#589`) hand-corrected in
  place, dashboard "Ambiguous" section updated (no claim those four were the only stale lines).
  **(b) Ranked combination shortlist (4 + a parked reserve):** (1) cross-μ `#608` generalization
  pilot (the `#542` closer — MEDIUM confidence it's decisive either way); (2) generalize `#610`'s
  interval bend-gate certificate to the ~6 other bend-gate-limited `empty_regions.jsonl` negatives
  (`#607`x4, `#609`, `#571`x2 — pure parameter substitution into the existing certified script,
  HIGH feasibility, upgrades the registry's main product to continuum-strength on the bend axis);
  (3) seed `#620`'s arc collocation from `#619`'s ~166,000 km ballistic near-miss — the arc's own
  explicitly-flagged last untried refinement (LOW ~10-15% closure odds, but payoff-asymmetric:
  closure = first genuine SE↔EM cislunar cycler after Fable review, failure = the `#538`-`#620`
  negative loses its last asterisk); (4) μ-continue the Ross-RT (k1,k2) ballistic-cycler families
  DOWN below their published μ=0.001 floor to Titan/Triton/Galilean/Uranian μ values (`#494`'s
  machinery is μ-agnostic and only ever went UP to binaries — HIGH feasibility, LOW-MEDIUM novelty:
  small-μ symmetric stable orbits shade into classical Poincaré first-kind families, so the
  literature gate is load-bearing; pilot 2 families at Titan μ first). **Rejected with reasons**
  (checked, don't re-propose): `#608`-seeds→`#620`-search (zero training examples of the needed
  object class — QBCP transfer arcs — anywhere; the wall is basin structure, not seed polish);
  QBCP re-pass of Galilean/Uranian/Saturnian sweeps (alphas are hardwired Sun-Earth-Moon AND those
  negatives are GM/geometry/novelty-limited, not fidelity-limited); bigger generative models at
  n=54k (`#614` already ran that comparison, clean negative). **Literature/methods:** a real
  CORPUS_INDEX pass found nothing new actionable; the one standing reserve is Wilczak-Zgliczyński
