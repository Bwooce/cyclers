...
- **#615** (dispatched 2026-07-16, `#613` follow-up, motivated by a direct re-read of the source
  ...
  doi-10.1007-s42064-024-0201-0.pdf` directly rather than trusting the existing digest alone:
  **(1) O&B's own §2.3 (p.4) states quasi-periodic tori at FIXED Jacobi constant form a
  1-PARAMETER family in rotation number** ("setting the Jacobi integral... constant, they then
  exist in 1-parameter families defined by their unique ratio of fundamental frequencies") — i.e.
  there is NOT one natural rotation number at C=3.15, there is a continuum. `#612`'s corrector
  PINS rho as a hard gauge constraint (needed to prevent collapse to a degenerate omega2->0
  branch) and only varies amplitude at the near-bifurcation linear rho estimate; `#613` varied `C`
  instead, again at the near-bifurcation linear estimate. **Neither ever explored the free-rho,
  fixed-C=3.15, large-amplitude branch of the family** that O&B's own framing describes — test
  whether 0.2739 is reachable there via a genuine pseudo-arclength continuation (step along the
  solution curve in (coeffs, omega1, omega2) space, not pinning rho to a specific value) rather
  than amplitude- or rho-pinned continuation.
  **(2) O&B's own illustrative Fig. 5 (p.6) uses an L2 SOUTHERN NEAR-RECTILINEAR HALO ORBIT
  (NRHO)** — a much larger-amplitude, far-from-bifurcation family member (motivated by the intro's
  explicit Gateway/Artemis framing), not the small-amplitude near-bifurcation halo every task in
  the `#555`-`#613` chain has assumed. The paper does not explicitly confirm the actual §4.1.1
  L1/L2 orbits are NRHOs, but it is untested and plausible — NRHOs have a genuinely different
  Floquet/bifurcation structure. Build the EM L1/L2 NRHO families (this codebase already has NRHO
  machinery, `#553`'s own scoping and `nrho_continuation.py`) and check their associated
  quasi-halo rotation numbers against O&B's 0.2739/0.02163 targets, independently of hypothesis
  (1).
  A clean negative on either or both hypotheses is an acceptable, informative result — this is
  exploratory verification of two specific, well-grounded leads, not guaranteed to pan out.
  **Recommended model: Opus** (pseudo-arclength continuation design + genuine numerical-methods
  judgment, same tier as `#612`).
