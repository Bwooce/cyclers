...
- **#612** (dispatched 2026-07-16, user-approved follow-up to `#606`/`#611`) — extend the seedless
  spectral/harmonic-balance method to `#556`'s parking-lot target: the large-rotation-number EM L1
  quasi-halo QUASI-PERIODIC 2-TORUS that `genome.qp_tori.correct_qp_torus` cannot converge above
  amp~0.01 (needed to carry the published latitudinal frequency 0.2739 near C=3.15, per `#555`'s
  Owen & Baresi reproduction attempt). Root-cause hypothesis to verify first: the existing GMOS/
  invariant-circle corrector resolves the LONGITUDINAL direction via actual stroboscopic FLOW
  INTEGRATION over `t_strob` (see `qp_tori.py`'s own docstring) — i.e. it still shoots/propagates,
  the same fragility class `#611` found in the QBCP case, just in CR3BP instead. If confirmed, the
  natural fix is the direct generalization `#611`'s own scope-boundary section named but declined
  as out-of-scope: a full 2D pseudospectral formulation — Fourier-expand BOTH angles
  `(theta_long, theta_trans)` with BOTH frequencies `(omega_long, omega_trans)` as free unknowns
  (the rotation number is their ratio), residual = `omega_long*d/dtheta_long x + omega_trans*d/
  dtheta_trans x - f(x)` at 2D collocation points — no forward integration anywhere in the search,
  mirroring exactly why `#606`/`#611` beat shooting-based correctors on their own walls. User
  explicitly approved dispatching this (2026-07-16), overriding `#556`'s standing "not auto-fired"
  flag for this specific harmonic-balance approach (the flag's original concerns — an open-ended
  numerical-methods fix, a corridor that's absorbed 5+ tasks — still apply and should inform scope
  discipline, not block the attempt). Positive-control target: reproduce the ALREADY-CONVERGED L2
  near-bifurcation case first (`#555`'s existing working L2 build, matching 0.02163 exactly) as a
  regression floor before attempting the harder L1 amp~0.02 case. **Recommended model: Opus** (per
  `#556`'s own note — this is a genuine diagnose-then-build judgment call, not mechanical
  implementation).
